...
class GaussControlDistGenerator(nn.Module):

    def __init__(self, d_model, q_fc, k_fc, clip_value_control=8/15, clip_value_std=10, dr_rate=0, ignore_std=False):
        super().__init__()
        self.d_model = d_model  # The dimension of the internal layers
        self.clip_value_control = clip_value_control  # Value used for clipping the attention scores
        self.clip_value_std = clip_value_std  # Value used for clipping the attention scores
        self.ignore_std = ignore_std  # If True, the std is not calculated; assigned to a constant valued tensor
    ...
    def calculate_attention(self, query, key, mask, mode='mean'):
        ...
        if mode == 'mean':
            # Apply clipping to the attention scores using the clip_value; [-control, +control]
            attention_score = self.clip_value_control * (torch.tanh(attention_score))
            neutral_action = 0
            if mask is not None:
                # Apply the mask to the attention scores
                attention_score = attention_score.masked_fill(mask == 0, neutral_action)
    ...
    def forward(self, *args, query, key, mask=None):
        ...
        attention_score_mean = self.calculate_attention(query_mean, key_mean, mask, mode='mean')
        if self.ignore_std:
            attention_score_std = torch.ones_like(attention_score_mean) * -10
        else:
            attention_score_std = self.calculate_attention(query_std, key_std, mask, mode='std')
