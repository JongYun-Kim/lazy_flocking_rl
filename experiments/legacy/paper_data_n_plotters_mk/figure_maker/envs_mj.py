...
class LazyAgentsCentralized(gym.Env):
    ...
    def reset(self):
        ...
        self.u_lazy = self.u_fully_active  # shape (num_agents_max,)
        # Compute reward
        rewards = self.compute_reward()
        
        self.target_reward = rewards[1] if self.use_L2_norm else rewards[0]
        self.initial_rewards = self.compute_auxiliary_reward(rewards=rewards, target_reward=self.target_reward,)
    ...
    def reset_for_test(self, test_agent:dict):

        test_pos = test_agent["pos"]
        test_ang = test_agent["ang"]
        test_omg = test_agent["omg"]

        # Reset agent number
        self.num_agents = np.random.randint(self.num_agents_min, self.num_agents_max + 1)
        self.num_agents += 1
        self.num_agents_min += 1
        self.num_agents_max += 1

        # Initialize padding
        self.is_padded = np.zeros(shape=(self.num_agents,), dtype=np.int32)
        self.is_padded[self.num_agents:] = 1

        # Initialize network topology
        self.net_topology_full = np.zeros((self.num_agents, self.num_agents), dtype=np.int32)
        self.update_topology_from_padding(init=True)

        # Initialize agent states
        # (1) position: shape (num_agents_max, 2)
        self.agent_pos = np.random.uniform(-self.l_bound / 2, self.l_bound / 2, (self.num_agents_max-1, 2))
        self.agent_pos = np.vstack((test_pos,self.agent_pos))
        # (2) angle: shape (num_agents_max, 1); it's a 2-D array !!!!
        self.agent_ang = np.random.uniform(-np.pi, np.pi, (self.num_agents_max-1, 1))
        self.agent_ang = np.vstack((test_ang, self.agent_ang))
        # (3) velocity v = [vx, vy] = [v*cos(theta), v*sin(theta)]; shape (num_agents_max, 2)
        self.agent_vel = self.v * np.concatenate((np.cos(self.agent_ang), np.sin(self.agent_ang)), axis=1)
        # (4) angular velocity; shape (num_agents_max, 1); it's a 2-D array !!!!
        self.agent_omg = np.zeros((self.num_agents_max-1, 1))
        self.agent_omg = np.vstack((test_omg, self.agent_omg))
    ...
    def single_step(self,
        ...
        self.target_reward = rewards[1] if self.use_L2_norm else rewards[0]
        # reward = -self.dt
        auxiliary_reward = self.compute_auxiliary_reward(rewards=rewards, target_reward=self.target_reward,)
        ...
        reward = auxiliary_reward if auxiliary_reward is not NotImplemented else self.target_reward
    ...
    def get_u(self, mask=None, get_decomposed_u=False):
        ...
        # Get mask
        mask = self.is_padded == 0 if mask is None else mask
        assert mask.dtype == np.bool_  # np.bool_ depending on your ray (or numpy) version
        assert np.sum(mask) == self.num_agents  # TODO: remove the asserts once the code is stable
